document.addEventListener("DOMContentLoaded", () => {
  ...
  heroSection.style.backgroundImage = `url('${img[currentImageIndex].src}')`;
  
  setInterval(() => {
    let nextImageIndex = (currentImageIndex + 1) % img.length;
    heroSection.style.setProperty(
      "--next-image",
      `url('${img[nextImageIndex].src}')`
    );
    ...
    heroSection.addEventListener(
      "transitionend",
      () => {
        heroSection.style.backgroundImage = `url('${img[nextImageIndex].src}')`;
